...
unsigned long previousMillis_LED_Pump = 0;
unsigned long previousMillis_LED_Pump_Waiting = 0;
...
long OnTime_LED_Pumping = 2000;
long OffTime_LED_Pumping = 200;

void LEDpumping()
{
	unsigned long currentMillis_LED_Pump = millis();
...
long OnTime_LED_Pump_Waiting = 75;
long OffTime_LED_Pump_Waiting = 2500;

void LEDpump_waiting()
{
	unsigned long currentMillis_LED_Pump_Waiting = millis();
...
void LEDs_Alerts()
{
	if ((current_state.current_FC >= current_sensor_value.cached_SMOp) && (current_state.current_SM <= current_state.SMmin && digitalRead(NoWater_PIN) == LOW)
           || (current_state.current_SM <= current_state.SMmin && digitalRead(NoWater_PIN) == HIGH)
           || ((current_state.current_SM <= current_state.SMmin && current_state.current_ST > current_state.STMax)) || ((current_state.current_SM <= current_state.SMmin && current_state.current_ST < current_state.STmin)))
        {
		LEDpump_waiting();
	}
	
	if (current_state.current_FC >= current_state.SMOp)
	{
		LEDFieldC_Reached();
	}
	else
...
int Dangerous_Soil_Temperature = current_state.current_ST >= current_state.STMax || current_state.current_ST <= current_state.STmin;
int Field_Capacity_Reached = current_state.current_FC >= current_state.SMOp;
int Dry_Soil = current_state.current_SM <= current_state.SMmin;
int Safety_Moisture_Range = current_state.current_SM > current_state.SMmin && current_state.current_SM < current_state.SMOp;
int Wet_Soil = current_state.current_SM >= current_state.SMOp;
...
void Pump_pulses()
{
	long Time_Pumping = current_sensor_value.cached_TICicle * current_sensor_value.cached_PICicle * 10;
	long Time_Waiting_Pumping = current_sensor_value.cached_TICicle * (1000 - current_sensor_value.cached_PICicle * 10);
