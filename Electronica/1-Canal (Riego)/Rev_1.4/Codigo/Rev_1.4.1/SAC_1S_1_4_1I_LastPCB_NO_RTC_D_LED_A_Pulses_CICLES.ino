...
void loop()
{
	...
	if (current_state.current_FC >= current_sensor_value.cached_SMOp)
	{
		LEDFieldC_Reached();
	}
	else
	...
	if ((current_state.current_SM <= current_state.SMmin && digitalRead(NoWater_PIN) == HIGH))
	{
		LEDpump_waiting();
	}

	if (((current_state.current_SM <= current_state.SMmin && current_state.current_ST > current_state.STMax)) || ((current_state.current_SM <= current_state.SMmin && current_state.current_ST < current_state.STmin)))
	{
		LEDpump_waiting();
	}
